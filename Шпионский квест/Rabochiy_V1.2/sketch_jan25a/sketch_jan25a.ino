...
int pomada = 10;     // пин кнопки, флешки, ручки-замыкатора
int knopka = A3;
int buttonState = 0;

int capPin = 9;
//#define SensorPin 9     //Датчик препятствия
...
{ 
  Serial.begin(9600);
    ...
    mp3_set_volume(27);     // устанавливаем громкость от 0 до 30
  ...
  pinMode(pomada, INPUT); // Инициализируем цифровой вход/выход в режиме входа.
...
if( digitalRead(capPin) == HIGH  && digitalRead(pomada) == HIGH ) // если кнопка тач и кнопка помада нажаты одновременно
{
    mp3_play(4);                                                  // играет музыка все ок
    digitalWrite( led_pin3, HIGH );                               // горит светодиод 
    servo2.write(90);                                              // поворот сервы отсека 2
    delay(5000);                                                   // задержка для музыки
    mp3_stop ();                                                   // стоп для музыки
    
}



if( digitalRead(capPin) == HIGH) // если нажата кнопка тач, играет звук неверной индификации
{
    mp3_play(3);
...
if(digitalRead(knopka)== HIGH)//если кнопка нажата ...
 ...
 { 
 digitalWrite(led_pin3,HIGH);//выключаем 
 servo1.write(90);
 }
}
...
if(digitalRead(pomada)== HIGH)//если кнопка нажата ...
 { 
 digitalWrite(led_pin1,LOW);//включаем светодиод 
 servo1.write(0);
 } 
